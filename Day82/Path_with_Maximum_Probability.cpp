...
public:
    double maxProbability(int n, vector<vector<int>>& edges,
        ...
        while (!nodesToExplore.empty()) {
            ...
            if (u == end)
                return prob;
            // If we've already explored this node with a higher probability,
            // skip it
            ...
            for (auto [v, edgeIdx] : graph[u]) {
                ...
                // Update the probability of the neighbor node if we've found a
                // higher probability
                if (newProb > probabilities[v]) {
                    probabilities[v] = newProb;

                    // Add the neighbor node to the priority queue with its new
                    // probability
                    nodesToExplore.push({newProb, v});
                }
            }
        }
        // If we've explored all nodes and haven't reached the end node, return
        // 0.0
        return 0.0;
    }
};